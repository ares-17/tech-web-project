        ...
        <form @submit.prevent="onConfirmQuestion">
            <div class="col-12">
                ...
                <div v-else class="w-100">
                    ...
                    <div
                        v-if="submitted && questionRef.answers?.every(a => a.isCorrect === false) && questionRef?.answers?.length === 4">
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        function noNull(value: any) {
            return (value !== null && value !== undefined) || 'Seleziona almeno una risposta come corretta'
        }
        ...
        function onClickRadio(index: any) {
            ...
            questionRef.value.answers = questionRef.value?.answers
                ?.map((a, i) => ({ ...a, isCorrect: i === index }));
        }
        ...
        function hasErrorsSingleAnswer(): boolean{
            return questionRef.value.answers?.length !== 1 ||
                questionRef.value.answers[0]?.text?.trim() === ''; 
        }

        function hasErrorsMultipleAnswers(): boolean | undefined{
            return questionRef.value!.answers!.length !== 4 ||
